// ============================================
// The field, new improved version  October. 2001, mod. Aug. 2004
// -----------------------------------------
// This is a slightly modified version of the demo file abyss.pov
// provided in the official POV-Ray distribution since 3.5
// -----------------------------------------
...
// colors
//--------------------------------------------
#declare C_Water1=rgb<0,79,159>/255;
#declare C_Water2=rgb<7,146,217>/255;
#declare C_Water3=rgb<82,239,238>/255;
#declare C_Sub=<7/255,146/255,217/255>;

//--------------------------------------------
// lights
//--------------------------------------------
light_source {<-10, 1000, -10> color C_Water2*10
#if (AreaOK)
    area_light x*200,z*200, 3,3 adaptive 1 jitter orient
#end

}
light_source {<-200, -1000, -300> color C_Water2*2 shadowless media_interaction off}
light_source {PdV color C_Water2*2 shadowless media_interaction off}

//--------------------------------------------
// mine textures
//--------------------------------------------
#declare T_Mine=texture {
    pigment{color C_Water3*0.1}
    finish{ambient 0 diffuse 0.4 specular 0.03 roughness 0.2 reflection 0.05}
}
#declare T_Cable=texture {
    pigment{color C_Water3*0.1}
...
#declare trb=0.0001;
#declare P_Lettre=pigment{bozo color_map{[0 White*1.3][1 White*0.5]}}
#declare T_Lettre=texture{ // submarine name
    pigment {
        object {
            text{ttf "cyrvetic.ttf" "PERSISTENCE" 10, 0.3*x
                 translate -z*0.5 scale <1,1,10>
            }
            pigment{color Clear}, pigment{P_Lettre}
        }
        rotate y*90
...
#declare T_Sub0=texture {
    pigment{rgb C_Sub*0.2}
...
#declare T_SubBase=texture {
    pigment {
        cells
        color_map {
            [.45 rgb <C_Sub.x*0.1,C_Sub.y*0.1,C_Sub.z*0.1>]
            [.55 rgb <C_Sub.x,C_Sub.y,C_Sub.z>*0.8]
        }
        scale <100,.125,1>
    }
    scale 3
...
#declare T_SubTop=
    ...
    texture {
        pigment {
            cells
            color_map {
                [.25 rgbf <C_Sub.x*0.1,C_Sub.y*0.1,C_Sub.z*0.1,0>]
                [.75 rgbf <C_Sub.x,C_Sub.y,C_Sub.z,1>]
            }
            scale <100,0.75,1>
        }
        scale 3.5
    ...
    texture {
        pigment {
            cells
            color_map {
                [.25 rgbf <C_Sub.x*0.4,C_Sub.y*0.4,C_Sub.z*0.4,0>]
                [.75 rgbf <C_Sub.x,C_Sub.y,C_Sub.z,1>]
            }
            scale <100,0.45,1>
        }
...
#declare T_SubBottom=
    ...
    texture {
        pigment {
            cells
            color_map {
                [.25 rgbf <C_Sub.x*0.5,C_Sub.y*0.5,C_Sub.z*0.5,0>]
                [.75 rgbf <C_Sub.x,C_Sub.y,C_Sub.z,1>]
            }
            scale <100,.75,1>
        }
    ...
    texture {
        pigment {
            cells
            color_map {
                [0 rgbf <C_Sub.x*0.5,C_Sub.y*0.5,C_Sub.z*0.5,.5>]
                [1 rgbf <C_Sub.x,C_Sub.y,C_Sub.z,1>]
            }
            scale <100,0.25,1>
        }
        scale 5
